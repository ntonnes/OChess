...
let render_chessboard renderer = 
  for row = 0 to 7 do
    for col = 0 to 7 do
      ...
      (* draw board square *)
      let rect = Sdl.Rect.create ~x:((col * !cell_size) + !offset_x) ~y:((row * !cell_size) + !offset_y) ~w:!cell_size ~h:!cell_size in
...
let render_pieces renderer game_state =
  for row = 0 to 7 do
    for col = 0 to 7 do
      ...
      let rect = Sdl.Rect.create ~x:((col * !cell_size)+10+ !offset_x) ~y:((row * !cell_size)+10+ !offset_y) ~w:(!cell_size-20) ~h:(!cell_size-20) in
...
let update window renderer game_state =
